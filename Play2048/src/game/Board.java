...
public class Board {
    ...
    public void addRandomTile() {
        ...
        BoardSpot selectile = openSpaces.get(t);

        double v = StdRandom.uniform(0.0,1.0);
        if(v < 0.1){
        gameBoard[selectile.getRow()][selectile.getCol()] = 4;
        }
        else {
            gameBoard[selectile.getRow()][selectile.getCol()] = 2;
        }
    ...
    public void swipeLeft() {
        for ( int r = 0; r < gameBoard.length; r++ ) {
            for ( int c = 0; c < gameBoard[r].length; c++ ) {
                for ( int d = 0; d < gameBoard[r].length; d++ ){
                    if (gameBoard[r][d]==0 && gameBoard[r][c] != 0 && d <c){
                        gameBoard[r][d] = gameBoard[r][c];
                        gameBoard[r][c]=0;
                    }
                }
            }
